...
async def echo_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Echo the photo message."""
    file = await update.message.photo[-1].get_file()
    await update.message.reply_photo(photo=file.file_id)

async def echo_document(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Echo the document message."""
    file = await update.message.document.get_file()
    await update.message.reply_document(document=file.file_id)
